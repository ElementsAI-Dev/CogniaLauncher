...
export type SecondarySidebarItemId =
  (typeof SECONDARY_SIDEBAR_ITEM_IDS)[number];
...
export function isPrimarySidebarItemId(
  value: SidebarItemId,
): value is PrimarySidebarItemId {
  return PRIMARY_ITEM_SET.has(value);
}

export function isSecondarySidebarItemId(
  value: SidebarItemId,
): value is SecondarySidebarItemId {
  return SECONDARY_ITEM_SET.has(value);
}

export function normalizeSidebarItemOrder(
