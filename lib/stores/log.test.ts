...
describe('useLogStore', () => {
  ...
  describe('getFilteredLogs', () => {
    ...
    it('should filter by target', () => {
      ...
      useLogStore.getState().addLogs([
        { timestamp: 1, level: 'info', message: 'msg1', target: 'app::core' },
        { timestamp: 2, level: 'info', message: 'msg2', target: 'app::network' },
        { timestamp: 3, level: 'info', message: 'msg3', target: 'lib::utils' },
      ]);
      useLogStore.getState().setFilter({ target: 'app' });
      const filtered = useLogStore.getState().getFilteredLogs();
      expect(filtered.length).toBe(2);
      expect(filtered[0].target).toBe('app::core');
      expect(filtered[1].target).toBe('app::network');
    });
    ...
    it('should search matching target field (plain text)', () => {
      useLogStore.setState({ logs: [] });
      useLogStore.getState().addLogs([
        { timestamp: 1, level: 'info', message: 'hello', target: 'my_module' },
        { timestamp: 2, level: 'info', message: 'world', target: 'other' },
      ]);
      useLogStore.getState().setSearch('my_module');
      const filtered = useLogStore.getState().getFilteredLogs();
      expect(filtered.length).toBe(1);
      expect(filtered[0].target).toBe('my_module');
    });

    it('should search matching target field (regex)', () => {
      useLogStore.setState({ logs: [] });
      useLogStore.getState().addLogs([
        { timestamp: 1, level: 'info', message: 'hello', target: 'app::core' },
        { timestamp: 2, level: 'info', message: 'world', target: 'lib::net' },
      ]);
      useLogStore.getState().setFilter({
        levels: ['info'],
        search: 'app::',
