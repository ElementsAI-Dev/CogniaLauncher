...
describe("useToolboxMarketplace", () => {
  ...
  it("marks listing provenance as degraded when installed store plugin lacks matching acquisition context", () => {
    const { result } = renderHook(() => useToolboxMarketplace());
    const listing = result.current.listings[0];
      ...
      installedPlugins: [
        {
          ...
          updatedAt: null,
          updateUrl: null,
          source: { type: "store" as const, storeId: listing.source.storeId },
    ...
    const { result: refreshed } = renderHook(() => useToolboxMarketplace());
    const resolvedListing = refreshed.current.listings.find(
      (entry) => entry.id === listing.id,
    );

    expect(resolvedListing?.installState).toBe("installed");
    expect(resolvedListing?.provenanceState).toBe("degraded");
    expect(resolvedListing?.provenanceReason).toBe(
