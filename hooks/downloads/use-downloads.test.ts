...
describe('useDownloads', () => {
  ...
  it('keeps runtime side effects single-owned even if multiple runtime consumers mount', async () => {
    renderHook(() => useDownloads());
    renderHook(() => useDownloads());

    await waitFor(() => {
      expect(mockListenDownloadTaskAdded).toHaveBeenCalledTimes(1);
    });

    expect(mockDownloadList).toHaveBeenCalledTimes(1);
    expect(mockDownloadStats).toHaveBeenCalledTimes(1);
    expect(mockDownloadHistoryList).toHaveBeenCalledTimes(1);
    expect(mockDownloadHistoryStats).toHaveBeenCalledTimes(1);
    expect(mockDownloadGetSpeedLimit).toHaveBeenCalledTimes(1);
    expect(mockDownloadGetMaxConcurrent).toHaveBeenCalledTimes(1);
  });
  ...
  it('checks destination availability through the shared download hook', async () => {
    const { result } = renderHook(() => useDownloads({ enableRuntime: false }));

    let available = false;
    await act(async () => {
      available = await result.current.checkDestinationAvailability('/downloads/file.zip');
    });
