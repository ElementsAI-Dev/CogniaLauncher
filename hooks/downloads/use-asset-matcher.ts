...
export interface ParsedAsset<TAsset extends AssetLike = GitHubAssetInfo> {
  asset: TAsset;
...
export function useAssetMatcher() {
  ...
  const parseAssets = useCallback(
    function parseAssets<TAsset extends AssetLike>(assets: TAsset[]): ParsedAsset<TAsset>[] {
      return assets
  ...
  const getRecommendedAsset = useCallback(
    function getRecommendedAsset<TAsset extends AssetLike>(assets: TAsset[]): TAsset | null {
