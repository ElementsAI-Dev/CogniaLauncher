...
describe('usePlugins', () => {
  ...
  it('should expose all actions', () => {
    ...
    expect(typeof result.current.cancelTool).toBe('function');
    ...
    expect(tool.sdkCapabilityCoverage).toEqual([
      expect.objectContaining({
        ...
        preferredWorkflow: expect.objectContaining({
          ...
          workflowIntents: expect.arrayContaining(['inspect-local-ports']),
        }),
      }),
    ]);
  ...
  it('normalizes structured tool execution errors from plugin runtime', async () => {
    tauri.isTauri.mockReturnValue(true);
    tauri.pluginCallTool.mockRejectedValue({
      kind: 'timeout',
    ...
    await expect(
      act(async () => result.current.callTool('plugin.test', 'entry', '{}')),
    ).rejects.toEqual({
      kind: 'timeout',
      message: 'request timed out',
    });
  ...
  it('forwards tool cancellation to tauri runtime', async () => {
    tauri.isTauri.mockReturnValue(true);
    tauri.pluginCancelTool.mockResolvedValue(true);

    const { result } = renderHook(() => usePlugins());

    await act(async () => {
      await result.current.cancelTool('exec-1');
    });

    expect(tauri.pluginCancelTool).toHaveBeenCalledWith('exec-1');
  });
