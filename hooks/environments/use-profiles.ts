...
import {
  ...
  profileCaptureWslSnapshot,
  profileApplyWslSnapshot,
  type EnvironmentProfile,
  type ProfileEnvironment,
  type ProfileApplyResult,
  type WslProfileSnapshot,
  type WslProfileApplyResult,
} from '@/lib/tauri';
...
interface UseProfilesReturn {
  ...
  captureWslSnapshot: () => Promise<WslProfileSnapshot | null>;
  applyWslSnapshot: (snapshot: WslProfileSnapshot) => Promise<WslProfileApplyResult | null>;
}
...
export function useProfiles(): UseProfilesReturn {
  ...
  const createProfile = useCallback(
    async (
      ...
      try {
        const result = await profileCreate(name, description, environments, options);
        await refresh();
        return result;
      } catch (err) {
        setError(err instanceof Error ? err.message : String(err));
  ...
  const createFromCurrent = useCallback(
    async (
      ...
      try {
        const result = await profileCreateFromCurrent(name, options);
        await refresh();
        return result;
      } catch (err) {
        setError(err instanceof Error ? err.message : String(err));
  ...
  const captureWslSnapshot = useCallback(async (): Promise<WslProfileSnapshot | null> => {
    if (!isTauri()) {
      return null;
    }

    try {
      return await profileCaptureWslSnapshot();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      return null;
    }
  ...
  const applyWslSnapshot = useCallback(async (
    snapshot: WslProfileSnapshot,
  ): Promise<WslProfileApplyResult | null> => {
    if (!isTauri()) {
      return null;
    }

    try {
      return await profileApplyWslSnapshot(snapshot);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      return null;
    }
