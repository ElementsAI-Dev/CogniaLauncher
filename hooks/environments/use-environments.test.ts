...
describe('useEnvironments', () => {
  ...
  it('reuses detected versions cache within TTL and refreshes after the TTL expires', async () => {
    const initialDetected = [
      { env_type: 'node', version: '20.10.0', source: '.nvmrc', source_path: '/project/.nvmrc' },
    ];
    const refreshedDetected = [
      { env_type: 'node', version: '20.11.0', source: '.nvmrc', source_path: '/project/.nvmrc' },
    ];
    let now = 0;
    const nowSpy = jest.spyOn(Date, 'now').mockImplementation(() => now);
    mockEnvDetectAll
      .mockResolvedValueOnce(initialDetected)
      .mockResolvedValueOnce(refreshedDetected);
    const { result } = renderHook(() => useEnvironments());

    let cachedDetected;
    let revalidatedDetected;
    await act(async () => {
    ...
    now = 5_000;
    await act(async () => {
      cachedDetected = await result.current.detectVersions('/project');
    });

    now = 11_000;
    await act(async () => {
      revalidatedDetected = await result.current.detectVersions('/project');
    });

    expect(mockEnvDetectAll).toHaveBeenCalledTimes(2);
    expect(cachedDetected).toEqual(initialDetected);
    expect(revalidatedDetected).toEqual(refreshedDetected);
    nowSpy.mockRestore();
  });
  ...
  it('prefers workflow context project path over persisted project path when saving env settings', async () => {
    window.localStorage.setItem('cognia-project-path', '/persisted/project');
    mockStoreState.workflowContext = {
      envType: 'node',
    ...
    mockEnvGetDetectionSources.mockResolvedValue(['.nvmrc']);
    mockEnvGetDefaultDetectionSources.mockResolvedValue(['.nvmrc']);
    const { result } = renderHook(() => useEnvironments());
    ...
    expect(mockEnvDetectAll).toHaveBeenCalledWith('/workflow/project');
    expect(mockStoreActions.setWorkflowAction).toHaveBeenLastCalledWith(
  ...
  it('falls back to persisted project path when workflow context is missing during settings save', async () => {
    window.localStorage.setItem('cognia-project-path', '/persisted/project');
    mockEnvGetDetectionSources.mockResolvedValue(['.nvmrc']);
    mockEnvGetDefaultDetectionSources.mockResolvedValue(['.nvmrc']);
    const { result } = renderHook(() => useEnvironments());
    ...
    expect(mockEnvDetectAll).toHaveBeenCalledWith('/persisted/project');
    expect(mockStoreActions.setWorkflowAction).toHaveBeenLastCalledWith(
      expect.objectContaining({
        envType: 'node',
        action: 'saveSettings',
        status: 'success',
        projectPath: '/persisted/project',
      }),
    );
  });
  ...
  it('dedupes concurrent provider fetches while the request is still in flight', async () => {
    const providers = [
      { id: 'fnm', display_name: 'fnm', description: 'Fast Node Manager', env_type: 'node' },
    ];
    let resolveProviders: ((value: typeof providers) => void) | null = null;
    mockEnvListProviders.mockImplementation(
      () => new Promise<typeof providers>((resolve) => {
    ...
    let secondResult;
    await act(async () => {
      const firstRequest = result.current.fetchProviders();
      const secondRequest = result.current.fetchProviders();
    ...
    expect(firstResult).toEqual(providers);
    expect(secondResult).toEqual(providers);
    expect(mockStoreActions.setAvailableProviders).toHaveBeenCalledWith(providers);
  });
