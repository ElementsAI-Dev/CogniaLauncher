...
describe("useEnvironmentWorkflow", () => {
  ...
  it("reuses persisted project path and current provider when syncing the same target", () => {
    ...
    act(() => {
      result.current.syncWorkflowContext("node", {
        origin: "detail",
      });
    });
  ...
  it("allows path-sensitive actions to proceed when PATH is already configured", () => {
    const { result } = renderHook(() => useEnvironmentWorkflow());

    let allowed = false;
    act(() => {
      allowed = result.current.requirePathConfigured({
        envType: "node",
        action: "setupPath",
        providerId: "fnm",
        projectPath: "/workspace/app",
        pathStatus: { isInPath: true },
        reason: "Already configured.",
      });
    });

    expect(allowed).toBe(true);
    expect(mockSetWorkflowAction).not.toHaveBeenCalled();
  });
  ...
  it("returns an empty reconciliation result outside Tauri", async () => {
    mockIsTauri.mockReturnValue(false);
    const { result } = renderHook(() => useEnvironmentWorkflow());

    let reconciled;
    await act(async () => {
      reconciled = await result.current.reconcileEnvironmentWorkflow({
        projectPath: "/workspace/app",
        refreshProviders: true,
      });
    });
  ...
  it("skips provider refresh and project detection when no project path is available", async () => {
    const envs = [{ env_type: "node", provider_id: "fnm" }];
    mockEnvList.mockResolvedValue(envs);
    const { result } = renderHook(() => useEnvironmentWorkflow());
