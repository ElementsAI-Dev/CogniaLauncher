...
jest.mock('@/lib/tauri', () => ({
  ...
  wslListPortForwards: (...args: unknown[]) => mockWslListPortForwards(...args),
}));
...
describe('useWsl information snapshots', () => {
  beforeEach(() => {
    ...
    mockWslListPortForwards.mockResolvedValue([
      {
  ...
  it('builds a ready runtime info snapshot from shared refresh orchestration', async () => {
    ...
    expect(result.current.runtimeInfo.running_count).toBe(1);
    expect(result.current.runtimeInfo.default_distro).toBe('Ubuntu');
    expect(result.current.runtimeInfo.kernel_up_to_date).toBe(true);
    expect(result.current.runtimeInfo.is_healthy).toBe(true);
  ...
  it('stores distro information snapshots for running distros', async () => {
    ...
    expect(result.current.distroInfoByName.Ubuntu.portForwards.data).toEqual([
      {
        listenAddress: '0.0.0.0',
        listenPort: '3000',
        connectAddress: '172.24.240.1',
