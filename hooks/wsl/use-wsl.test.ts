...
const mockWslSetNetworkingMode = jest.fn();
...
const mockWslExportWindowsEnv = jest.fn();
const mockWslReadDistroEnv = jest.fn();
const mockWslGetWslenv = jest.fn();
const mockWslSetWslenv = jest.fn();
...
jest.mock('@/lib/tauri', () => ({
  ...
  wslSetNetworkingMode: (...a: unknown[]) => mockWslSetNetworkingMode(...a),
  traySetWslState: (...a: unknown[]) => mockTraySetWslState(...a),
  trayRebuild: (...a: unknown[]) => mockTrayRebuild(...a),
  ...
  wslExportWindowsEnv: (...a: unknown[]) => mockWslExportWindowsEnv(...a),
  wslReadDistroEnv: (...a: unknown[]) => mockWslReadDistroEnv(...a),
  wslGetWslenv: (...a: unknown[]) => mockWslGetWslenv(...a),
  wslSetWslenv: (...a: unknown[]) => mockWslSetWslenv(...a),
...
describe('useWsl', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useWslStore.setState({ backupSchedules: [] });
  ...
  it('should terminate distro', async () => {
    ...
    mockWslListRunning.mockResolvedValue([]);
    mockWslGetStatus.mockResolvedValue({ defaultDistribution: 'Ubuntu', runningDistros: [] });
    ...
    expect(mockTraySetWslState).toHaveBeenCalledWith(0, 'Ubuntu');
  ...
  it('should launch distro', async () => {
    ...
    mockWslListDistros.mockResolvedValue([]);
    mockWslListRunning.mockResolvedValue(['Ubuntu']);
    mockWslGetStatus.mockResolvedValue({ defaultDistribution: 'Ubuntu', runningDistros: ['Ubuntu'] });
  ...
  it('should set networking mode and refresh config', async () => {
    mockWslSetNetworkingMode.mockResolvedValue(undefined);
    mockWslGetConfig.mockResolvedValue({ wsl2: { networkingMode: 'mirrored' } });

    const { result } = renderHook(() => useWsl());

    await act(async () => {
      await result.current.setNetworkingMode('mirrored');
    });

    expect(mockWslSetNetworkingMode).toHaveBeenCalledWith('mirrored');
  ...
  it('should export Windows env into a distro script', async () => {
    const exportResult = {
      distro: 'Ubuntu',
      targetPath: '~/.cognia_env',
      variableCount: 3,
    };
    mockWslExportWindowsEnv.mockResolvedValue(exportResult);

    const { result } = renderHook(() => useWsl());

    let res;
    await act(async () => {
      res = await result.current.exportWindowsEnv('Ubuntu');
    });

    expect(res).toEqual(exportResult);
    expect(mockWslExportWindowsEnv).toHaveBeenCalledWith('Ubuntu');
  });

  it('should read distro env variables', async () => {
    const envResult = {
      distro: 'Ubuntu',
      variables: [
    ...
    mockWslReadDistroEnv.mockResolvedValue(envResult);

    const { result } = renderHook(() => useWsl());

    let res;
    await act(async () => {
      res = await result.current.readDistroEnv('Ubuntu');
    });

    expect(res).toEqual(envResult);
    expect(mockWslReadDistroEnv).toHaveBeenCalledWith('Ubuntu');
  });

  it('should get and set WSLENV entries', async () => {
    const entries = [
      { key: 'GOPATH', flags: ['p'] },
      { key: 'JAVA_HOME', flags: ['p', 'u'] },
    ];
    mockWslGetWslenv.mockResolvedValue(entries);
    mockWslSetWslenv.mockResolvedValue(undefined);

    const { result } = renderHook(() => useWsl());

    let res;
    await act(async () => {
      res = await result.current.getWslenv();
    });

    expect(res).toEqual(entries);

    await act(async () => {
      await result.current.setWslenv(entries);
    });

    expect(mockWslSetWslenv).toHaveBeenCalledWith(entries);
  });
  ...
  it('polls backup schedules and updates next run after a due backup', async () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date('2026-03-29T09:00:00.000Z'));

    mockWslBackupDistro.mockResolvedValue({
      fileName: 'Ubuntu-2026-03-29.tar',
      filePath: 'C:\\WSL-Backups\\Ubuntu-2026-03-29.tar',
      sizeBytes: 1024,
      createdAt: '2026-03-29T09:00:00.000Z',
      distroName: 'Ubuntu',
    });
    mockWslListBackups.mockResolvedValue([
      {
        fileName: 'Ubuntu-2026-03-29.tar',
        filePath: 'C:\\WSL-Backups\\Ubuntu-2026-03-29.tar',
        sizeBytes: 1024,
      ...
      backupSchedules: [
        {
          ...
          retention: 2,
          last_run: null,
          next_run: '2026-03-29T09:00:00.000Z',
        },
      ],
    });
    ...
    expect(mockWslBackupDistro).toHaveBeenCalledWith('Ubuntu', '%USERPROFILE%\\WSL-Backups');
    expect(useWslStore.getState().backupSchedules[0].last_run).toBe('2026-03-29T09:00:00.000Z');
    expect(useWslStore.getState().backupSchedules[0].next_run).toBe('2026-03-30T01:00:00.000Z');

    jest.useRealTimers();
  });

  it('runs missed backup detection immediately on mount', async () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date('2026-03-29T09:00:00.000Z'));
      ...
      backupSchedules: [
        {
          ...
          retention: 2,
          last_run: null,
          next_run: '2026-03-28T09:00:00.000Z',
        },
      ],
    });
  ...
  it('cleans up stale backups beyond retention after a scheduled backup succeeds', async () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date('2026-03-29T09:00:00.000Z'));

    mockWslBackupDistro.mockResolvedValue({
      fileName: 'Ubuntu-new.tar',
      filePath: 'C:\\WSL-Backups\\Ubuntu-new.tar',
      sizeBytes: 1024,
      createdAt: '2026-03-29T09:00:00.000Z',
      distroName: 'Ubuntu',
    });
    mockWslListBackups.mockResolvedValue([
      ...
      {
        fileName: 'Ubuntu-old.tar',
        filePath: 'C:\\WSL-Backups\\Ubuntu-old.tar',
        sizeBytes: 1024,
    ...
    expect(mockWslDeleteBackup).toHaveBeenCalledWith('C:\\WSL-Backups\\Ubuntu-old.tar');
    jest.useRealTimers();
  });
});
