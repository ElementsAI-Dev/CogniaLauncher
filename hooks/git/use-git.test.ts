...
describe('useGit', () => {
  ...
  it('allows force push when explicit confirmation is provided', async () => {
    const { result } = renderHook(() => useGit());
    await act(async () => { await result.current.setRepoPath('/repo'); });

    await act(async () => {
      await result.current.push('origin', 'main', true, undefined, undefined, true);
    });

    expect(tauri.gitPush).toHaveBeenCalledWith(
      '/repo',
      'origin',
      'main',
      true,
    ...
    expect(tauri.gitGetStatus).toHaveBeenCalled();
    expect(tauri.gitGetBranches).toHaveBeenCalled();
    expect(tauri.gitGetAheadBehind).toHaveBeenCalled();
  });
