...
const mockBatchInstall = jest.fn();
...
const mockPreInstallValidate = jest.fn();
const mockResolveDependencyConflict = jest.fn();
...
jest.mock('@/lib/tauri', () => ({
  ...
  batchInstall: (...args: Parameters<typeof mockBatchInstall>) => mockBatchInstall(...args),
  ...
  preInstallValidate: (...args: Parameters<typeof mockPreInstallValidate>) => mockPreInstallValidate(...args),
  resolveDependencyConflict: (...args: Parameters<typeof mockResolveDependencyConflict>) => mockResolveDependencyConflict(...args),
  pluginDispatchEvent: (...args: Parameters<typeof mockPluginDispatchEvent>) => mockPluginDispatchEvent(...args),
}));

jest.mock('@/lib/cache/invalidation', () => ({
  emitInvalidations: (...args: Parameters<typeof mockEmitInvalidations>) => mockEmitInvalidations(...args),
  ensureCacheInvalidationBridge: jest.fn(),
  subscribeInvalidation: jest.fn(() => jest.fn()),
  withThrottle: (fn: (...args: unknown[]) => void) => fn,
}));
...
const mockSetPendingConflicts = jest.fn();
const mockSetResolvedVersion = jest.fn();
...
const mockStoreActions = {
  ...
  setPendingConflicts: (conflicts: unknown[]) => {
    mockSetPendingConflicts(conflicts);
    mockStoreState.pendingConflicts = conflicts ?? [];
  },
  setResolvedVersion: (packageName: string, version: string) => {
    mockSetResolvedVersion(packageName, version);
    mockStoreState.resolvedVersions = {
      ...mockStoreState.resolvedVersions,
      [packageName]: version,
    };
  },
...
describe('usePackages', () => {
  beforeEach(() => {
    ...
    mockStoreState.pendingConflicts = [];
    mockStoreState.resolvedVersions = {};
    mockStoreState.isLoading = false;
    mockStoreState.error = null;
    mockIsScanFresh.mockReturnValue(false);
    mockPreInstallValidate.mockResolvedValue({
      results: [],
      can_proceed: true,
      has_warnings: false,
      has_failures: false,
  ...
  it('should return package methods', () => {
    ...
    expect(result.current).toHaveProperty('batchInstall');
  ...
  it('should install packages', async () => {
    ...
    expect(mockPreInstallValidate).toHaveBeenCalledWith(['react']);
    expect(mockPackageInstall).toHaveBeenCalledWith(['react']);
    expect(mockEmitInvalidations).toHaveBeenCalledWith(
      ['package_data', 'provider_data', 'environment_data', 'cache_overview', 'about_cache_stats'],
      'packages:install',
    );
  });

  it('surfaces preflight warnings and waits for confirmation before installing', async () => {
    mockPreInstallValidate.mockResolvedValue({
      results: [
        {
    ...
    await act(async () => {
      installPromise = result.current.installPackages(['react']);
    ...
    await act(async () => {
      result.current.confirmPreflight();
  ...
  it('blocks installation when preflight validation fails', async () => {
    mockPreInstallValidate.mockResolvedValue({
      results: [
        {
          validator_id: 'disk_space',
          validator_name: 'Disk space',
          status: 'failure',
          summary: 'Not enough free disk space.',
          details: ['Need at least 100 MB free space.'],
          remediation: 'Free disk space and retry.',
          package: 'react',
          provider_id: 'npm',
          blocking: true,
          timed_out: false,
    ...
    await act(async () => {
      await expect(result.current.installPackages(['react'])).rejects.toThrow(
  ...
  it('surfaces preflight warnings and waits for confirmation before batch installing', async () => {
    mockPreInstallValidate.mockResolvedValue({
      results: [
        {
          validator_id: 'provider_health',
    ...
    await act(async () => {
      batchPromise = result.current.batchInstall(['react']);
      await Promise.resolve();
    });

    expect(mockPreInstallValidate).toHaveBeenCalledWith(['react']);
    expect(result.current.isPreflightOpen).toBe(true);
    expect(mockBatchInstall).not.toHaveBeenCalled();
    ...
    expect(mockBatchInstall).toHaveBeenCalledWith(['react'], undefined);
    expect(result.current.isPreflightOpen).toBe(false);
  });

  it('blocks batch installation when preflight validation fails', async () => {
    mockPreInstallValidate.mockResolvedValue({
      results: [
        {
          validator_id: 'disk_space',
    ...
    await act(async () => {
      await expect(result.current.batchInstall(['react'])).rejects.toThrow(
        'Pre-flight validation failed',
      );
    });
  ...
  it('stores resolved versions after resolving dependency conflicts', async () => {
    mockResolveDependencyConflict.mockResolvedValue({
      success: true,
      ...
      recommendations: [
        {
          ...
          provider_id: 'pip',
          selected_version: '2.0.7',
          available_versions: ['1.26.18', '2.0.7'],
          strategy: 'latest_compatible',
          reason: 'Selected the newest available version.',
        },
      ],
      unresolved: [],
      message: null,
    ...
    let resolution;
    await act(async () => {
      resolution = await result.current.resolveConflicts([
        {
          package_name: 'urllib3',
    ...
    expect(mockResolveDependencyConflict).toHaveBeenCalled();
    expect(mockSetResolvedVersion).toHaveBeenCalledWith('urllib3', '2.0.7');
    expect(resolution).toEqual(expect.objectContaining({ success: true }));
  });

  it('emits environment invalidation after batch install completes', async () => {
    mockBatchInstall.mockResolvedValue({
      successful: [],
      failed: [],
      skipped: [],
    ...
    await act(async () => {
      await result.current.batchInstall(['react']);
    });
