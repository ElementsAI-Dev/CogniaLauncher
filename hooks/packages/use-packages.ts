...
import { usePackageUpdates, type CheckPackageUpdatesOptions } from '@/hooks/packages/use-package-updates';
import {
...
export function usePackages() {
  ...
  const pendingPreflightResolverRef = useRef<PendingPreflightResolver | null>(null);
  const [preflightSummary, setPreflightSummary] = useState<tauri.PackagePreflightSummary | null>(null);
  const [preflightPackages, setPreflightPackages] = useState<string[]>([]);
  ...
  const clearPreflightState = useCallback(() => {
    ...
    setPreflightPackages([]);
  }, []);

  const confirmPreflight = useCallback(() => {
    pendingPreflightResolverRef.current?.resolve(true);
    pendingPreflightResolverRef.current = null;
    clearPreflightState();
  }, [clearPreflightState]);

  const dismissPreflight = useCallback(() => {
    pendingPreflightResolverRef.current?.resolve(false);
    pendingPreflightResolverRef.current = null;
    clearPreflightState();
  }, [clearPreflightState]);

  const validateBeforeInstall = useCallback(async (packages: string[]) => {
    const summary = await tauri.preInstallValidate(packages);
    setPreflightSummary(summary);
    setPreflightPackages(packages);

    if (summary.has_failures || summary.has_warnings) {
    ...
    const shouldProceed = await new Promise<boolean>((resolve, reject) => {
      pendingPreflightResolverRef.current = { resolve, reject };
    });

    if (!shouldProceed) {
      throw new Error('Installation cancelled after pre-flight');
    }
  ...
  const installPackages = useCallback(async (packages: string[]) => {
    ...
    try {
      await validateBeforeInstall(packages);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchInstalledPackages, validateBeforeInstall]);
  ...
  const resolveDependencies = useCallback(async (packageName: string) => {
    store.setLoading(true);
    try {
      const result = await tauri.resolveDependencies([packageName]);
      store.setPendingConflicts(result.conflicts);
      return result;
    } catch (err) {
      store.setError(formatError(err));
      return null;
  ...
  const resolveConflicts = useCallback(async (
    conflicts: tauri.ConflictInfo[],
    strategy: tauri.ConflictResolutionStrategy,
    manualVersions?: Record<string, string>,
  ) => {
    try {
      const result = await tauri.resolveDependencyConflict(
        conflicts,
        strategy,
        manualVersions,
      );
      store.setPendingConflicts(
        conflicts.filter(
          (conflict) =>
            !result.recommendations.some(
              (recommendation) =>
                recommendation.package_name === conflict.package_name,
            ),
        ),
      );
      result.recommendations.forEach((recommendation) => {
        store.setResolvedVersion(
          recommendation.package_name,
          recommendation.selected_version,
        );
      });
      return result;
    } catch (err) {
