...
const mockInstallWithPreflight = jest.fn();
...
let mockPreflightSummary: unknown = null;
...
jest.mock('@/hooks/packages/use-packages', () => ({
  usePackages: () => ({
    ...
    installPackages: (...args: unknown[]) => mockInstallWithPreflight(...args),
    preflightSummary: mockPreflightSummary,
    preflightPackages: mockPreflightPackages,
    isPreflightOpen: mockIsPreflightOpen,
    confirmPreflight: (...args: unknown[]) => mockConfirmPreflight(...args),
    dismissPreflight: (...args: unknown[]) => mockDismissPreflight(...args),
  }),
}));
...
describe('useProviderDetail', () => {
  ...
  it('should initialize with default state', () => {
    ...
    expect(result.current.preflightSummary).toBeNull();
    expect(result.current.preflightPackages).toEqual([]);
  ...
  it('should install package', async () => {
    mockInstallWithPreflight.mockResolvedValue(undefined);
    ...
    expect(mockInstallWithPreflight).toHaveBeenCalledWith(['npm:express@4.18.0']);
  ...
  it('should install package without version', async () => {
    ...
    expect(mockInstallWithPreflight).toHaveBeenCalledWith(['npm:express']);
  });

  it('should expose shared pre-flight state and actions', async () => {
    ...
    expect(result.current.preflightSummary).toEqual(mockPreflightSummary);
    expect(result.current.preflightPackages).toEqual(['npm:express']);
    expect(result.current.isPreflightOpen).toBe(true);

    act(() => {
      result.current.confirmPreflight();
      result.current.dismissPreflight();
    });
