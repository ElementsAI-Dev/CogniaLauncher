...
import { usePackageUpdates } from '@/hooks/packages/use-package-updates';
...
export function useProviderDetail(providerId: string) {
  ...
  const {
    ...
    installPackages: installWithPreflight,
    preflightSummary,
    preflightPackages,
    isPreflightOpen,
    confirmPreflight,
  ...
  const installPackage = useCallback(async (packageName: string, version?: string) => {
    try {
      ...
      await installWithPreflight([spec]);
  ...
  }, [installWithPreflight, providerId, refreshPackageSurface]);
