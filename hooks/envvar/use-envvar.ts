...
const envVarOverviewQueryClient = new QueryClient({
  defaultOptions: {
    queries: {
      staleTime: 30_000,
      gcTime: 5 * 60_000,
...
const ENVVAR_OVERVIEW_QUERY_KEY = ['envvar', 'overview'] as const;

export function __resetEnvVarOverviewCache() {
  envVarOverviewQueryClient.removeQueries({ queryKey: ENVVAR_OVERVIEW_QUERY_KEY, exact: true });
}
...
export function useEnvVar() {
  const envvarDefaultScope = useSettingsStore((state) => state.appSettings.envvarDefaultScope);
  const envvarAutoSnapshot = useSettingsStore((state) => state.appSettings.envvarAutoSnapshot);
  const envvarMaskSensitive = useSettingsStore((state) => state.appSettings.envvarMaskSensitive);
  ...
  const autoRevealedKeysRef = useRef(new Set<string>());
  ...
  const invalidateOverviewCache = useCallback(() => {
    void envVarOverviewQueryClient.invalidateQueries({ queryKey: ENVVAR_OVERVIEW_QUERY_KEY });
  }, []);

  const registerAutoSnapshotResult = useCallback((result: EnvVarSnapshotCreateResult | null) => {
    if (!result?.snapshot) {
      return;
    }
    setState((s) => ({
      ...
      snapshotHistory: [
        result.snapshot!,
        ...s.snapshotHistory.filter((item) => item.path !== result.snapshot!.path),
      ],
  ...
  const getOverview = useCallback(async (
    options?: { forceRefresh?: boolean },
  ): Promise<EnvVarOverview | null> => {
    const forceRefresh = options?.forceRefresh ?? false;
    const cached = envVarOverviewQueryClient.getQueryData<EnvVarOverview>(ENVVAR_OVERVIEW_QUERY_KEY);

    setState((s) => ({
      ...s,
      overview: cached ?? s.overview,
    ...
    try {
      ...
      const overview = await envVarOverviewQueryClient.fetchQuery({
        queryKey: ENVVAR_OVERVIEW_QUERY_KEY,
        queryFn: () => tauri.envvarGetOverview(),
  ...
  const reconcileRecoveryState = useCallback(async (
    options?: {
      detectionScope?: EnvVarDetectionScope;
      pathScope?: EnvVarScope;
      clearImportPreview?: boolean;
      clearPathRepairPreview?: boolean;
      refreshSupportSnapshot?: boolean;
      refreshSnapshotHistory?: boolean;
    },
  ) => {
    const detectionScope = options?.detectionScope ?? 'all';
    const clearImportPreviewState = options?.clearImportPreview ?? false;
    const clearPathRepairPreviewState = options?.clearPathRepairPreview ?? false;
    const refreshSupportSnapshot = options?.refreshSupportSnapshot ?? true;
    const refreshSnapshotHistory = options?.refreshSnapshotHistory ?? true;

    invalidateDetectionCache(detectionScope);
    invalidateOverviewCache();

    const [detection, overview, supportSnapshot, snapshotHistory, pathEntries] = await Promise.all([
      loadDetection(detectionScope, { forceRefresh: true }),
      getOverview({ forceRefresh: true }),
      refreshSupportSnapshot
        ? tauri.envvarGetSupportSnapshot().catch((err) => {
            const message = formatError(err);
            setState((s) => ({ ...s, supportLoading: false, supportError: message }));
            return undefined;
          })
      ...
      refreshSnapshotHistory
        ? tauri.envvarListSnapshots().catch((err) => {
            const message = formatError(err);
            setState((s) => ({ ...s, snapshotLoading: false, snapshotError: message }));
            return undefined;
          })
        : Promise.resolve(undefined),
      options?.pathScope
        ? tauri.envvarGetPath(options.pathScope).catch((err) => {
    ...
    setState((s) => ({
      ...
      ...(snapshotHistory !== undefined
        ...
        : {}),
      ...(pathEntries !== undefined ? { pathEntries } : {}),
      importPreview: clearImportPreviewState ? null : s.importPreview,
      importPreviewStale: clearImportPreviewState ? false : s.importPreviewStale,
      pathRepairPreview: clearPathRepairPreviewState ? null : s.pathRepairPreview,
      pathRepairPreviewStale: clearPathRepairPreviewState ? false : s.pathRepairPreviewStale,
    }));

    return { detection, overview, supportSnapshot, snapshotHistory, pathEntries };
  }, [getOverview, invalidateDetectionCache, invalidateOverviewCache, loadDetection, setError]);

  const maybePrepareProtectedMutation = useCallback(async (
    action: string,
    scopes: EnvVarScope[],
  ): Promise<boolean> => {
    const recoveryAction = getCanonicalRecoveryAction(action);
    const persistentScopes = Array.from(
      new Set(scopes.filter((scope) => scope === 'user' || scope === 'system')),
    );

    if (persistentScopes.length === 0) {
      return true;
    }

    try {
      const protectionStates = await Promise.all(
        persistentScopes.map((scope) => tauri.envvarGetBackupProtection(recoveryAction, scope)),
      );
      const blockedState = protectionStates.find((state) => state.state === 'blocked');
      if (blockedState) {
        setError(blockedState.reason || blockedState.reasonCode || 'Recovery protection blocked');
        return false;
      }

      if (!envvarAutoSnapshot) {
        return true;
      }

      const requiresSnapshot = protectionStates.some((state) => state.state === 'will_create');
      if (!requiresSnapshot) {
        return true;
      }
      ...
      const result = await tauri.envvarCreateSnapshot(
        ...
        'automatic',
        recoveryAction,
        `Auto snapshot before ${recoveryAction}`,
      );
      registerAutoSnapshotResult(result);
      if (!result.success) {
        setError(result.message ?? 'Auto snapshot failed');
        return false;
      }
      return true;
    } catch (err) {
  ...
  }, [envvarAutoSnapshot, registerAutoSnapshotResult, setError]);

  const refreshAfterScopeMutation = useCallback(async (scope: EnvVarScope) => {
    await reconcileRecoveryState({
      detectionScope: getDetectionRefreshScope(scope),
    });
  }, [getDetectionRefreshScope, reconcileRecoveryState]);

  const refreshAfterPathMutation = useCallback(async (scope: EnvVarScope) => {
    const result = await reconcileRecoveryState({
      detectionScope: getDetectionRefreshScope(scope),
      pathScope: scope,
    });
    return result.pathEntries ?? [];
  }, [getDetectionRefreshScope, reconcileRecoveryState]);
  ...
  useEffect(() => {
    if (envvarMaskSensitive) {
      if (autoRevealedKeysRef.current.size === 0) {
        return;
      }
      setState((s) => {
        const nextRevealedValues = { ...s.revealedValues };
        for (const key of autoRevealedKeysRef.current) {
          delete nextRevealedValues[key];
        }
        return { ...s, revealedValues: nextRevealedValues };
      });
      autoRevealedKeysRef.current.clear();
      return;
    }

    const revealTargets: Array<{ key: string; scope: EnvVarScope }> = [];
    const collectTargets = (
      scope: EnvVarScope,
      summaries: EnvVarSummary[],
    ) => {
      for (const item of summaries) {
        const cacheKey = `${scope}:${item.key}`;
        if (!item.value.masked || state.revealedValues[cacheKey] != null) {
          continue;
        }
        revealTargets.push({ key: item.key, scope });
      }
    };

    collectTargets('process', state.processVarSummaries);
    collectTargets('user', state.userPersistentVarSummaries);
    collectTargets('system', state.systemPersistentVarSummaries);
    ...
    void (async () => {
      for (const target of revealTargets) {
        ...
        const cacheKey = `${target.scope}:${target.key}`;
        try {
          const result = await tauri.envvarRevealValue(target.key, target.scope);
          if (cancelled || result.value == null) {
            continue;
          }
          autoRevealedKeysRef.current.add(cacheKey);
          setState((s) => ({
            ...s,
            revealedValues: {
              ...s.revealedValues,
              [cacheKey]: result.value!,
            },
          }));
        } catch {
          // Keep default masked display when auto-reveal is unavailable.
        }
      }
    })();
    ...
    envvarMaskSensitive,
    state.processVarSummaries,
    state.revealedValues,
    state.systemPersistentVarSummaries,
    state.userPersistentVarSummaries,
  ]);
  ...
  const restoreSnapshot = useCallback(async (
    ...
    setState((s) => ({ ...s, snapshotLoading: true, snapshotError: null }));
    try {
      const result = await tauri.envvarRestoreSnapshot(snapshotPath, scopes, previewFingerprint);
      if (result.success) {
        await reconcileRecoveryState({
          detectionScope: 'all',
          clearImportPreview: true,
          clearPathRepairPreview: true,
        });
        setState((s) => ({
          ...s,
          snapshotLoading: false,
  ...
  const setVar = useCallback(async (
    ...
    try {
      ...
      if (scope === 'process') {
        ...
        const canProceed = await maybePrepareProtectedMutation('persistent_set', [scope]);
        if (!canProceed) {
          return null;
        }
  ...
  }, [invalidateDetectionCache, invalidateOverviewCache, maybePrepareProtectedMutation, setError]);
  ...
  const removeVar = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('persistent_remove', [scope]);
      if (!canProceed) {
        return null;
      }
      let rawResult: EnvVarMutationResult | boolean | undefined;
  ...
  const addPathEntry = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('path_add', [scope]);
      if (!canProceed) {
        return null;
      }
  ...
  }, [maybePrepareProtectedMutation, refreshAfterPathMutation, setError]);
  ...
  const removePathEntry = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('path_remove', [scope]);
      if (!canProceed) {
        return null;
      }
  ...
  const reorderPath = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('path_reorder', [scope]);
      if (!canProceed) {
        return null;
      }
  ...
  const applyImportPreview = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('import_apply', [scope]);
      if (!canProceed) {
        return null;
      }
  ...
  }, [isStalePreviewError, maybePrepareProtectedMutation, refreshAfterScopeMutation, setError]);
  ...
  const deduplicatePath = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('path_deduplicate', [scope]);
      if (!canProceed) {
        return null;
      }
  ...
  const applyPathRepair = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('path_repair_apply', [scope]);
      if (!canProceed) {
        return null;
      }
  ...
  }, [isStalePreviewError, maybePrepareProtectedMutation, refreshAfterPathMutation, setError]);
  ...
  const resolveConflict = useCallback(async (
    ...
    setError(null);
    try {
      const canProceed = await maybePrepareProtectedMutation('conflict_resolve', [targetScope]);
      if (!canProceed) {
        return null;
      }
  ...
  }, [clearRevealedVar, maybePrepareProtectedMutation, refreshAfterScopeMutation, setError]);
  ...
  return {
    ...
    getOverview,
    getActionSupport,
    defaultScopePreference: envvarDefaultScope,
    autoSnapshotEnabled: envvarAutoSnapshot,
    maskSensitiveByDefault: envvarMaskSensitive,
  };
}
