...
const mockTerminalCreateProfile = jest.fn();
const mockTerminalUpdateProfile = jest.fn();
const mockTerminalDeleteProfile = jest.fn();
const mockTerminalSetDefaultProfile = jest.fn();
const mockTerminalDetectFramework = jest.fn();
const mockTerminalListPlugins = jest.fn();
...
jest.mock('@/lib/tauri', () => ({
  ...
  terminalCreateProfile: (...args: unknown[]) => mockTerminalCreateProfile(...args),
  terminalUpdateProfile: (...args: unknown[]) => mockTerminalUpdateProfile(...args),
  terminalDeleteProfile: (...args: unknown[]) => mockTerminalDeleteProfile(...args),
  terminalSetDefaultProfile: (...args: unknown[]) => mockTerminalSetDefaultProfile(...args),
  ...
  terminalListPlugins: (...args: unknown[]) => mockTerminalListPlugins(...args),
...
describe('useTerminal', () => {
  beforeEach(() => {
    ...
    mockTerminalCreateProfile.mockResolvedValue('created-profile');
    mockTerminalUpdateProfile.mockResolvedValue(undefined);
    mockTerminalDeleteProfile.mockResolvedValue(true);
    mockTerminalSetDefaultProfile.mockResolvedValue(undefined);
    mockTerminalDetectFramework.mockResolvedValue([]);
    mockTerminalListPlugins.mockResolvedValue([]);
  ...
  it('launchProfile updates launching state and stores detailed result', async () => {
    ...
    expect(useTerminalStore.getState().recentlyLaunchedIds).toEqual(['profile-1']);
    expect(toast.success).toHaveBeenCalledWith('terminal.toastProfileLaunched');
  });

  it('writes created profiles through to the shared terminal store', async () => {
    const { result } = renderHook(() => useTerminal({ t: mockT }));
    await waitFor(() => expect(mockTerminalDetectShells).toHaveBeenCalledTimes(1));
    ...
    expect(useTerminalStore.getState().profiles).toEqual([
  ...
  it('updates and deletes profiles through the shared terminal store', async () => {
    useTerminalStore.setState({
      profiles: [
        {
          id: 'profile-1',
    ...
    expect(useTerminalStore.getState().profiles[0].name).toBe('Updated');
  ...
  it('syncs default profile changes into the shared terminal store', async () => {
    const profile = {
      id: 'profile-1',
    ...
    mockTerminalListProfiles.mockResolvedValue([profile]);
    useTerminalStore.setState({
      profiles: [profile],
      shells: [],
      defaultProfileId: null,
    ...
    await act(async () => {
      await result.current.setDefaultProfile('profile-1');
    });

    expect(useTerminalStore.getState().defaultProfileId).toBe('profile-1');
  ...
  it('marks startup readout as stale when re-measure fails after a successful result', async () => {
    mockTerminalMeasureStartup
      .mockResolvedValueOnce({
        shellId: 'bash',
        withProfileMs: 120,
        withoutProfileMs: 80,
      ...
      .mockRejectedValueOnce(new Error('startup probe unavailable'));
    mockTerminalDetectShells.mockResolvedValue([
      {
        id: 'bash',
        name: 'Bash',
        ...
        version: '5.2.0',
        executablePath: '/bin/bash',
        configFiles: [{ path: '/home/user/.bashrc', exists: true, sizeBytes: 128 }],
        isDefault: true,
      },
    ]);
    ...
    await act(async () => {
      await result.current.measureStartup('bash');
    });

    expect(result.current.startupMeasurements.bash?.withProfileMs).toBe(120);
    expect(result.current.shellReadouts.bash).toEqual(
      expect.objectContaining({
        shellId: 'bash',
        status: 'ready',
    ...
    expect(result.current.shellReadouts.bash).toEqual(
      expect.objectContaining({
        ...
        degradedReason: 'Error: startup probe unavailable',
      }),
    );
  });

  it('marks framework discovery as stale when refresh fails after a previous successful detection', async () => {
    mockTerminalDetectShells.mockResolvedValue([
      {
        id: 'bash',
        name: 'Bash',
    ...
    mockTerminalDetectFramework
      .mockResolvedValueOnce([
        {
          ...
          description: 'Bash plugin manager',
          homepage: null,
      ...
      .mockRejectedValueOnce(new Error('framework scan failed'));
    ...
    expect(result.current.shellReadouts.bash).toEqual(
      expect.objectContaining({
        ...
        status: 'stale',
        frameworkSummaryCount: 1,
        degradedReason: 'Error: framework scan failed',
      }),
    );
  });

  it('marks plugin readout as stale when plugin refresh fails after a previous successful load', async () => {
    mockTerminalDetectShells.mockResolvedValue([
      {
        id: 'zsh',
        name: 'Zsh',
        ...
        configFiles: [{ path: '/home/user/.zshrc', exists: true, sizeBytes: 512 }],
        isDefault: true,
      },
    ]);
    mockTerminalDetectFramework.mockResolvedValue([
    ...
    mockTerminalListPlugins
      .mockResolvedValueOnce([
        { name: 'git', enabled: true, source: 'oh-my-zsh' },
        { name: 'docker', enabled: false, source: 'oh-my-zsh' },
      ])
      .mockRejectedValueOnce(new Error('plugin registry unavailable'));

    const { result } = renderHook(() => useTerminal({ t: mockT }));
    await waitFor(() => expect(mockTerminalDetectShells).toHaveBeenCalledTimes(1));

    await act(async () => {
      await result.current.detectFrameworks('zsh');
    });
    await act(async () => {
      await result.current.fetchPlugins('Oh My Zsh', '/home/user/.oh-my-zsh', 'zsh', '/home/user/.zshrc');
    });

    const key = 'zsh:Oh My Zsh:/home/user/.oh-my-zsh';
    expect(result.current.frameworkReadouts[key]).toEqual(
      expect.objectContaining({
        status: 'ready',
        pluginCount: 2,
        freshness: 'fresh',
    ...
    expect(result.current.frameworkReadouts[key]).toEqual(
      expect.objectContaining({
        ...
        pluginCount: 2,
        freshness: 'stale',
        degradedReason: 'Error: plugin registry unavailable',
      }),
    );
    expect(result.current.shellReadouts.zsh).toEqual(
      expect.objectContaining({
