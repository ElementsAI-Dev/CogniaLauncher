...
Provider 管理相关界面现在统一使用同一套结构化检测状态，包括 Provider 列表页、Provider 详情页，以及包管理页中的内嵌 Provider 管理入口。

- `available`：Provider 已启用，且运行时探测成功。
- `unavailable`：Provider 当前不可用。被禁用的 Provider 也会显示为这个状态，直到再次启用。
- `timeout`：Provider 探测超出了超时预算。
- `unsupported`：Provider 在当前运行环境或平台范围内不受支持。

状态检查会尽量保留原因语义，因此 `timeout` 和 `unsupported` 不再被降维成简单的布尔结果。对 Provider 执行启用、禁用、优先级调整后，刷新和状态检查入口会从同一套后端契约重新拉取最新管理状态。
