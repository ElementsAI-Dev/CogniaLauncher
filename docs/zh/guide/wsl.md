...
- **关闭 WSL** — 关闭所有运行中的发行版
- **查看运行中** — 列出当前运行的发行版
- **运行时信息快照** — 运行时概览会以统一快照维护状态、能力和版本读取结果，并显示刷新时间与降级/陈旧提示
- **更完整的组件版本信息** — 运行时概览通过同一刷新链路展示 WSL、内核、WSLg 与 Windows 组件版本
...
- 运行时与发行版详情的信息读取现在通过共享快照统一管理，而不是零散的一次性请求。
- 发行版概览中的各个信息分区会明确展示 `不可用`、`数据陈旧` 与刷新重试提示，而不再静默清空。
- 如果刷新在曾经成功读取之后失败，CogniaLauncher 会保留最近一次成功的数据并标记为陈旧，方便你在不丢失上下文的情况下重试。
