...
| ID | 名称 | 类型 | 能力 |
|----|------|------|------|
| `zig` | Zig | 版本管理器 | Install, Uninstall, Search, List, VersionSwitch, MultiVersion, Update, ProjectLocal |
...
- Node 生态 Provider 会统一解析树形列表输出（如 `├── pkg@1.0.0` / `└── @scope/pkg@1.0.0`）。
- 安装列表解析时会忽略非包噪声行（banner、路径、计数行等）。
- Yarn/Bun 全局列表结果会先按 `name@version` 去重再返回。
