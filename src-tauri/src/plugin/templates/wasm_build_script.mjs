...
const WITH_BUNDLE = __WITH_BUNDLE__;
const WITH_INTERFACE = __WITH_INTERFACE__;
...
async function main() {
  ...
  } else if (!hasBinaryen(getSearchPaths())) {
  ...
  const inputFile = WITH_BUNDLE ? 'dist/plugin.js' : 'src/index.js';
  const compileArgs = [inputFile];
  if (WITH_INTERFACE) {
    compileArgs.push('-i', 'plugin.d.ts');
  }
  compileArgs.push('-o', 'plugin.wasm');
  ...
  runCommand(extismExecutable, compileArgs, { env });
...
async function ensureLocalBinaryen() {
  ...
  runCommand(tarBinary, ['-xzf', archivePath, '-C', extractRoot]);
...
async function downloadBuffer(url) {
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'CogniaLauncher-plugin-scaffold',
    },
  });
