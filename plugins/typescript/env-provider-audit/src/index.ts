...
const DEFAULT_ENV_TYPES = ['node', 'python', 'rust'];
...
function parseInput(raw: string): Required<EnvProviderAuditInput> {
  if (!raw.trim()) {
    return {
  ...
  return {
    envTypes,
    includeProviders: normalizeBoolean(candidate.includeProviders, true, 'includeProviders'),
    notifyOnIssues: normalizeBoolean(candidate.notifyOnIssues, false, 'notifyOnIssues'),
...
function normalizeEnvTypes(value: unknown): string[] {
  ...
  if (normalized.length === 0) {
    throw new AuditInputError('envTypes must include at least one environment type.');
  }
...
function runAudit(input: Required<EnvProviderAuditInput>): EnvProviderAuditSuccess {
  const platform = cognia.platform.info();
  const knownEnvironmentIds = new Set(cognia.env.list().map((entry) => entry.id));
  const providers = input.includeProviders
    ? cognia.env.providerList().map((provider) => ({
      id: provider.id,
  ...
  const issues = summarizeIssues(environments);
  const recommendations = buildRecommendations(environments, providers, issues.length);
  const pluginId = safeGetPluginId();

  if (input.emitEvent) {
    cognia.event.emit('builtin.env_provider_audit.completed', {
      pluginId,
  ...
  if (input.notifyOnIssues && issues.length > 0) {
    cognia.notification.send('Environment audit found issues', `${issues.length} issue(s) detected in ${environments.length} checks.`);
  }
...
function summarizeIssues(environments: EnvironmentSnapshot[]): string[] {
  const issues: string[] = [];
...
function buildRecommendations(
  ...
  if (providers.length === 0) {
    items.push('Enable includeProviders=true to inspect provider capabilities for deeper diagnostics.');
  }
  if (environments.some((env) => env.declaredByHost && !env.available)) {
    items.push('A host-declared environment is unavailable; verify provider binaries and PATH configuration.');
  }

  return items;
}

function safeGetPluginId(): string | null {
  try {
...
function buildFailure(error: unknown): EnvProviderAuditFailure {
  ...
  return {
    ...
    message: error instanceof Error ? error.message : 'Environment provider audit failed.',
    recommendations: [
