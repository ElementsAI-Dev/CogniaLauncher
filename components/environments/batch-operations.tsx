...
export function EnvironmentBatchOperations({
  ...
  }, [operationType, selectedVersions, onBatchInstall, onBatchUninstall, onClearSelection]);
  ...
  const getOperationLabel = () => {
    return operationType === 'install' ? t('common.install') : t('common.uninstall');
  };
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card border rounded-lg shadow-lg p-3 flex items-center gap-4 z-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            {t('environments.batch.selected', { count: selectedVersions.length })}
          </span>
        </div>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {result 
                ...
                : t('environments.batch.description', { action: getOperationLabel(), count: selectedVersions.length })
              }
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            {!result && !isProcessing && (
              <div className="space-y-2">
                <div className="text-sm font-medium">{t('environments.batch.versions')}:</div>
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="text-center">
                    <div className="font-medium">
                      {t('environments.batch.processing')} ({currentIndex + 1}/{selectedVersions.length})
                    </div>
                    <div className="text-sm text-muted-foreground mt-1">
                      {selectedVersions[currentIndex]?.envType} {selectedVersions[currentIndex]?.version}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{t('environments.batch.successful')}</div>
                  </div>
                  <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{t('environments.batch.failed')}</div>
                  </div>
                </div>
                    ...
                    {result.failed.map((item, i) => (
                      <div key={i} className="p-2 rounded bg-red-500/5 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline">{item.version.envType}</Badge>
                          <span className="text-sm font-mono">{item.version.version}</span>
                        </div>
          ...
          <DialogFooter>
            {!result && !isProcessing && (
              <>
                ...
                <Button onClick={handleOperation} variant={operationType === 'uninstall' ? 'destructive' : 'default'}>
                  {getOperationIcon()}
