...
export function LogFileViewer({ open, fileName, onOpenChange }: LogFileViewerProps) {
  const { t } = useLocale();
  ...
  }), [fileName, filter.endTime, filter.levels, filter.search, filter.startTime, filter.useRegex]);

  const mapEntry = useCallback((entry: { timestamp: string; level: string; target: string; message: string; lineNumber: number; }): UiLogEntry => ({
  ...
  const loadEntries = useCallback(async (offset = 0, append = false) => {
    ...
    try {
      ...
      setEntries((prev) => append ? [...nextEntries, ...prev] : nextEntries);
