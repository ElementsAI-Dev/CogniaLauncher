    ...
    try {
      ...
      switch (operationType) {
        ...
        case "uninstall":
          ...
          break;
        case "update":
          batchResult = await onBatchUpdate(
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {result
                ? t("packages.batchCompleted", {
                    time: (result.total_time_ms / 1000).toFixed(2),
                  })
                : t("packages.batchDescription", {
            ...
            {result && (
              <div className="space-y-4">
                ...
                {result.failed.length > 0 &&
                  result.failed.some((f) => f.recoverable) && (
                    <Alert>
                      <AlertCircle className="h-4 w-4" />
                      <AlertTitle>
                        {t("packages.someOperationsFailed")}
                      </AlertTitle>
                      <AlertDescription className="flex items-center justify-between">
                        <span>
                          {t("packages.operationsCanRetry", {
                            count: result.failed.filter((f) => f.recoverable)
                              .length,
                          })}
                        </span>
                        <Button
