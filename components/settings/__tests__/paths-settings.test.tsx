...
describe('PathsSettings', () => {
  ...
  it('should render paths settings card', () => {
    ...
    expect(screen.getByText('Override default storage locations')).toBeInTheDocument();
  });
  ...
  it('should display current path values', () => {
    ...
    expect(screen.getByLabelText('Environments Directory')).toHaveValue('/data/envs');
  });
  ...
  it('should call onValueChange when paths change', () => {
    ...
    fireEvent.change(screen.getByLabelText('Root Directory'), { target: { value: '/new/root' } });
