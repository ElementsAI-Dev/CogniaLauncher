...
describe('validateField', () => {
  describe('numeric validation', () => {
    it('should return null for valid parallel downloads value', () => {
      expect(validateField('general.parallel_downloads', '4', mockT)).toBeNull();
      expect(validateField('general.parallel_downloads', '1', mockT)).toBeNull();
      expect(validateField('general.parallel_downloads', '16', mockT)).toBeNull();
    });
    ...
    it('should validate network timeout range', () => {
      ...
      expect(validateField('network.timeout', '4', mockT)).toBe('Minimum value is 5');
      expect(validateField('network.timeout', '400', mockT)).toBe('Maximum value is 300');
    });

    it('should validate network retries range', () => {
      expect(validateField('network.retries', '3', mockT)).toBeNull();
      expect(validateField('network.retries', '-1', mockT)).toBe('Minimum value is 0');
      expect(validateField('network.retries', '15', mockT)).toBe('Maximum value is 10');
    });

    it('should validate metadata cache TTL range', () => {
      expect(validateField('general.metadata_cache_ttl', '3600', mockT)).toBeNull();
  ...
  describe('URL pattern validation', () => {
    it('should return null for valid proxy URL', () => {
      expect(validateField('network.proxy', 'http://localhost:8080', mockT)).toBeNull();
      expect(validateField('network.proxy', 'https://proxy.example.com', mockT)).toBeNull();
      expect(validateField('network.proxy', '', mockT)).toBeNull(); // Empty is allowed
    });

    it('should return error for invalid proxy URL', () => {
      expect(validateField('network.proxy', 'not-a-url', mockT)).toBe('Must be a valid URL or empty');
    });
    ...
    it('should validate NPM registry URL', () => {
      ...
      expect(validateField('mirrors.npm', 'invalid', mockT)).toBe('Must be a valid URL');
    });

    it('should validate PyPI index URL', () => {
      expect(validateField('mirrors.pypi', 'https://pypi.org/simple', mockT)).toBeNull();
      expect(validateField('mirrors.pypi', 'ftp://invalid', mockT)).toBe('Must be a valid URL');
    });
  });
...
describe('SettingItem', () => {
  ...
  it('should apply min and max attributes for number inputs', () => {
    render(
      <SettingItem {...defaultProps} type="number" value="5" min={1} max={10} />
    );
