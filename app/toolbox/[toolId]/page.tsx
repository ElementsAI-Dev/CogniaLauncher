...
export default function ToolDetailPage() {
  ...
  // toolId in URL can be: "json-formatter" (built-in shorthand) or full unified ID
  const tool = useMemo(() => {
    if (!params.toolId) return undefined;
    // Try exact match first
    const exact = allTools.find((t) => t.id === params.toolId);
    if (exact) return exact;
    // Try built-in shorthand match
    return allTools.find((t) => t.isBuiltIn && t.builtInDef?.id === params.toolId);
  }, [params.toolId, allTools]);
